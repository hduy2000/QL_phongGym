...
if (isset($_GET['id'])) {
	$id = $_GET['id'];
	$edit ="select * from tt_the where id=".$id;
	$agree = executeresult($edit);
	foreach ($agree as $ag) {
		$code = $ag['mathe'];
		$name = $ag['tenthe'];
		$common = $ag['loaithe'];
				...
				<div class="giahan col-8">
					<table class="table">
						...
						<tr>
							...
							<td>1</td>
							<td><input type="text" name="mathe" value="<?=$code?>"></td>
							<td><input type="text" name="mathe" value="<?=$name?>"></td>
							<td><input type="text" name="mathe" value="<?=$common?>"></td>
							<td><input type="text" name="mathe" value="<?=$price?>"></td>
							</form>
						</tr>
						<?php
							$i =2;
							$sqli = "select * from tt_the where id !=".$id;
